...
The record of how this project got here: what was built, what closed each
milestone, and — as often as not — what turned out to be wrong about the last
thing written down.

[roadmap.md](roadmap.md) is what is left to do. [state.md](state.md) is what is
true of the build right now, facts only.

**How to read it.** It is chronological, and *later sections supersede earlier
ones* — the headings say where each thing ended up. A finding that was later
contradicted is left in place with the correction under it, because knowing which
way a wrong answer was wrong is most of what makes the next one cheaper. Struck
text is a claim that did not survive.

## The milestones

| | | what closed it |
|---|---|---|
| [M0](#m0--foundation) | foundation | ✅ the SH-2 decoder agrees with `sh-elf-objdump` on all 53,752 valid encodings |
| [M1](#m1--the-sh-2-front-end) | SH-2 front end | ✅ `emit_asm.py --verify` reassembles the SH-2 image byte for byte, JU and E |
| [M2](#m2--the-68000-front-end) | 68000 front end | ✅ `disasm68k.py emit --verify` reassembles all 3,145,728 bytes, JU and E |
| [M3](#m3--sh-2--c-recompiler) | SH-2 → C recompiler | ✅ 8 semantics cases against values derived independently of any SH-2 model |
| [M4](#m4--runtime-skeleton) | runtime skeleton | ✅ `diff68k.py` walks the whole boot in step with the reference |
| [M5](#m5--first-pixels) | first pixels | ✅ the 32X picture at 300 frames is the SEGA logo, drawn correctly |
| [M6](#m6--sound) | sound | ✅ `diffpwm.py` sample for sample, and every byte reaching the two Mega Drive chips is the reference's |
| [M7](#m7--it-plays) | it plays | 🔵 in progress — a level can be finished and the special stage is playable |

## The landmarks

The dozen findings that moved the project, for anyone reading this to learn what
kind of bug a project like this actually has:

* [The 32X image is the SH-2's own](#the-32x-image-is-the-sh-2s-own) — the first
  picture that came out of recompiled code rather than around it.
* [Half the front end was not reachable, and only `--recomp` could
  tell](#half-the-front-end-was-not-reachable-and-only---recomp-could-tell) —
  562 functions were really 4,491.
* [The picture was reading its palette out of 68000
  code](#the-picture-was-reading-its-palette-out-of-68000-code) — one `uint16_t`
  serving two different registers on two different CPUs.
* [The SH-2s were running at half speed, and the logs said so in one
  command](#the-sh-2s-were-running-at-half-speed-and-the-logs-said-so-in-one-command).
* [Past the logo the game stopped moving, and the register was eight bits
  wide](#past-the-logo-the-game-stopped-moving-and-the-register-was-eight-bits-wide).
* [The vertex buffer came through a divide that never
  happened](#the-vertex-buffer-came-through-a-divide-that-never-happened) — the
  SH7604's on-chip divide unit was plain storage.
* [A computed return, a banked window, and the game
  plays](#a-computed-return-a-banked-window-and-the-game-plays).
* [One delay slot, and everything the 32X could not
  draw](#one-delay-slot-and-everything-the-32x-could-not-draw) — one address in
  the whole image, and two thirds of what the 32X draws.
* [The special stage, and the interrupt nobody
  delivered](#the-special-stage-and-the-interrupt-nobody-delivered).
* [Five more, each a different way of being
  unreachable](#five-more-each-a-different-way-of-being-unreachable) — a length
  bound is not a confidence bound.
* [The thirteenth missing block is a callback only a `mova`
...
*Where those three went.* The 14 escaping branches were tail transfers rather
than indirect dispatches, and modelling tail transfers properly subsumed them —
see [M4](#m4--runtime-skeleton). The hole in the PR model turned out to be real
and to be reached only after the title screen: `0x06000A64`, a literal that an
`lds` puts into PR, found in [A computed return, a banked window, and the game
plays](#a-computed-return-a-banked-window-and-the-game-plays). Literal pool loads
still go through memory, and nothing since has revisited it; it remains an
optimisation rather than a defect.

## M4 — runtime skeleton

*Both CPUs running, in step with the reference.*
...
*Both halves draw, and the 32X image is the SH-2's own.*
...
## After M5 — the runtime becomes accurate

*Not a milestone: the stretch between the first picture and the first sound,
where the front ends, the recompilers, the oracles and the clocks all had to
become right rather than merely present.*
...
and not audio — see [roadmap.md](roadmap.md). The other half of this milestone,
...
*A level can be finished and the special stage is playable; the reference gates
still stop at 1.7 seconds.*
...
does not need the reference. The instruments now exist; nothing
fails a build for them yet.

*Since written:* `tools/test_session.py` is that gate for one recorded play
session, and it does fail a build — see [The gate that needs no reference, and
says when it measured nothing](#the-gate-that-needs-no-reference-and-says-when-it-measured-nothing)
below. What is still unwritten is in [roadmap.md](roadmap.md).
...
*Carried through the sessions above and closed. Struck text is what was believed
at the time; what follows it is what turned out to be true. The debts still open
