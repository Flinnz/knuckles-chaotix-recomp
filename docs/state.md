...
Generated from the build at commit `6d306b7`. `make check` passes.
...
| 68000 whole extract (`diff68k.py`) | 51,782 of 54,081, 361 divergences |
...
| Z80 (`diffz80.py`) | 32,782 of 33,984, 315 divergences |
| chip register input (`diffz80.py`) | 164 of 164 bytes, in order |
| recompiled 68000, blocks (`diff68k.py --blocks`) | 4,847 of 9,848 blocks, 493 divergences |
...
| ~1,800 | title screen: five characters over the scrolling tunnel |
| ~2,400 | attract mode, first level |
| ~12,000 | attract loop continues, second level |
| ~17,000 | stops — commands frozen at 13,337, VRAM and CRAM cleared, 68000 PC `0xFFFFDD32` |

Commands a frame is 0.83 while healthy, which is the reference's rate.
...
| `--dump-vdp FILE`, `--dump-32x FILE`, `--dump-z80 FILE` | memory snapshots |
...
1. No gate exists past 1.7 seconds. Three oracles are available without a
   reference: the two 68000 backends against each other, liveness assertions,
   and the command rate.
2. The frontier is frame ~17,000.
...
Also open: `0x0749C2`-`0x0749D0` is 68000 code discovery has never found; the
recompiled 68000's PC is not masked to 24 bits; the Genesis VDP has no window
plane, shadow/highlight, interlace or per-line sprite limits; Genesis DMA fill
and copy are skipped.
